...
class MichelAndNBlobSB: public Study
{
  private:
    std::vector<EventVariable*> fEVars_Michel;
    std::vector<EventVariable*> fEVars_NBlobs;
    std::vector<EventVariable*> fEVars_Sel;
    std::vector<Variable*> fVars_Michel;
    std::vector<Variable*> fVars_NBlobs;

  public:
    MichelAndNBlobSB(std::vector<Variable*> vars,
	...
	new EventVariable("MaxEFSNeutKE","KE [MeV]", myBlobEBins, &NeutronEvent::GetMaxFSNeutronKE),
      ...
      fEVars_Michel = {
	new EventVariable("EMnBlobs_MichelSB","No.", myBlobNBins, &NeutronEvent::GetEMNBlobs),
	new EventVariable("EMBlobE_MichelSB","E [MeV]", myBlobEBins, &NeutronEvent::GetEMBlobE),
	new EventVariable("EMBlobNHit_MichelSB","No.", myBlobNHitBins, &NeutronEvent::GetEMBlobNHits),
	new EventVariable("EMBlobENHitRatio_MichelSB","E/No. [MeV]", myBlobERatBins, &NeutronEvent::GetEMBlobENHitRatio),
	new EventVariable("MaxEFSNeutKE_MichelSB","KE [MeV]", myBlobEBins, &NeutronEvent::GetMaxFSNeutronKE),
      };
	...
	new EventVariable("MaxEFSNeutKE_NBlobsSB","KE [MeV]", myBlobEBins, &NeutronEvent::GetMaxFSNeutronKE),
      ...
      for(auto& var: fVars_Michel) var->InitializeMCHists(mc_error_bands, truth_error_bands);
      for(auto& var: fVars_Michel) var->InitializeDATAHists(data_error_bands);

      for(auto& var: fEVars_Michel) var->InitializeMCHists(mc_error_bands, truth_error_bands);
      for(auto& var: fEVars_Michel) var->InitializeDATAHists(data_error_bands);
    ...
    {
      for (auto& var : fVars_Michel) var->WriteMC(outFile);
      for (auto& var : fVars_NBlobs) var->WriteMC(outFile);
      for (auto& var : fEVars_Michel) var->WriteMC(outFile);
    ...
    {
      for (auto& var : fVars_Michel) var->WriteData(outFile);
      for (auto& var : fVars_NBlobs) var->WriteData(outFile);
      for (auto& var : fEVars_Michel) var->WriteData(outFile);
